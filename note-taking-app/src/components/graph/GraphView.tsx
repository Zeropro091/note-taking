...
export default function GraphView({
  ...
  // ⚡ Bolt: Memoize graph data computation and use Set for O(1) node lookups
  // Reduces time complexity of edge filtering from O(N*E) to O(N+E)
  const currentData: FGData = useMemo(() => {
    let validNodes: Set<string> | null = null;
    ...
    return {
      nodes: graph.nodes
        .filter((node) => !filterTag || validNodes?.has(node.id))
        .map((node) => ({
          id: node.id,
          name: node.label,
