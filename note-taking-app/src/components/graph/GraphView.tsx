...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // ⚡ Bolt optimization: Precompute valid nodes to change O(N*E) edge filtering to O(N+E)
    ...
    return {
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag) return true;
          // ⚡ Bolt optimization: O(1) Set lookup instead of O(N) Array.find() inside O(E) loop
