...
export default function GraphView({
  ...
  // Memoize graph data processing to prevent O(N*E) recalculations on hover state changes
  const currentData: FGData = useMemo(() => {
    // 1. Pre-filter and map nodes
    const filteredNodes = graph.nodes.filter((node) => {
      if (!filterTag) return true;
      return node.tags.includes(filterTag);
    });

    // 2. Build a Set of valid node IDs for O(1) lookup during edge filtering
    // This avoids O(N) find() operations per edge
    ...
    return {
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag) return true;
          // Optimised check: An edge is valid if EITHER source or target is in our filtered node set
