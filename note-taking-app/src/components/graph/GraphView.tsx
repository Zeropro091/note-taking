...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // ⚡ Bolt: Pre-compute matching node IDs for O(1) lookups during edge filtering
    // Reduces edge filtering time complexity from O(V*E) to O(V+E)
    ...
    return {
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag) return true;
          // ⚡ Bolt: Fast Set lookup instead of multiple .find() array scans
