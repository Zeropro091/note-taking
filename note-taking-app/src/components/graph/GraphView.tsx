...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // Precompute node tags map to avoid O(N^2) lookups during edge filtering
    ...
    return {
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag) return true;
          const sourceTags = nodeTagsMap.get(edge.source) || [];
          const targetTags = nodeTagsMap.get(edge.target) || [];
          return (
