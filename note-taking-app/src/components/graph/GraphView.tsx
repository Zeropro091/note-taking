...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // ⚡ Bolt: Use a Set to store matching node IDs for O(1) lookup
    // This reduces link filtering time complexity from O(V*E) to O(V+E)
    ...
    const links = graph.edges
      .filter((edge) => {
        ...
        // O(1) lookup instead of O(V) array.find()
        return (
  ...
  // Get all unique tags
  // ⚡ Bolt: Memoize allTags to prevent unnecessary recalculation on every render
