...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // ⚡ Bolt: Precompute node tags lookup map for O(1) access instead of O(N) .find() inside .filter()
    // This improves time complexity from O(N*E) to O(N+E) for edge filtering
    const nodeTagMap = new Map<string, string[]>();
    graph.nodes.forEach(node => nodeTagMap.set(node.id, node.tags));
    ...
    return {
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag) return true;
          const sourceTags = nodeTagMap.get(edge.source) || [];
          const targetTags = nodeTagMap.get(edge.target) || [];
          return (
            sourceTags.includes(filterTag) || targetTags.includes(filterTag)
          );
        })
        .map((edge) => ({
          source: edge.source,
