...
export default function GraphView({
  ...
  // Save current graph data before isolation, wrapped in useMemo for performance
  const currentData: FGData = useMemo(() => {
    // Determine the visible set of nodes to optimize edge lookup (O(N) instead of O(N*E))
    const visibleNodeIds = new Set<string>();

    const nodes = graph.nodes
      .filter((node) => {
        const isVisible = !filterTag || node.tags.includes(filterTag);
        if (isVisible) {
    ...
    const links = graph.edges
      .filter((edge) => {
        ...
        // Optimization: O(1) Set lookup instead of O(N) array find
