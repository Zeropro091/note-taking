...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // Optimization: Precompute valid node IDs for O(1) lookups during edge filtering
    // This reduces filtering complexity from O(N*E) to O(N+E)
    ...
    return {
      nodes: graph.nodes
        .filter((node) => !filterTag || validNodeIds.has(node.id))
        .map((node) => ({
          id: node.id,
          name: node.label,
      ...
      links: graph.edges
        .filter((edge) => !filterTag || validNodeIds.has(edge.source) || validNodeIds.has(edge.target))
        .map((edge) => ({
          source: edge.source,
          target: edge.target,
        })),
