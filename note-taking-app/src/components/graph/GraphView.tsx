...
export default function GraphView({
  ...
  // ⚡ Bolt: Optimize edge filtering by pre-computing a Set of matching node IDs.
  // This reduces the time complexity from O(V*E) to O(V+E) by avoiding Array.prototype.find() on every edge.
  const currentData: FGData = useMemo(() => {
    const matchingNodeIds = filterTag
      ? new Set(
          graph.nodes
    ...
    return {
      nodes: graph.nodes
        .filter((node) => {
          ...
          return matchingNodeIds?.has(node.id) ?? false;
        })
        .map((node) => ({
          id: node.id,
          name: node.label,
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag) return true;
          return (
            (matchingNodeIds?.has(edge.source) ?? false) ||
            (matchingNodeIds?.has(edge.target) ?? false)
          );
        })
        .map((edge) => ({
          source: edge.source,
