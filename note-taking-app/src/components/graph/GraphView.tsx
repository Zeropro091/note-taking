...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // Precompute filtered nodes to avoid O(N*E) lookups in edges filtering
    const validNodeIds = new Set<string>();

    const nodes = graph.nodes
      .filter((node) => {
        const isValid = !filterTag || node.tags.includes(filterTag);
        if (isValid) validNodeIds.add(node.id);
        return isValid;
      })
      .map((node) => ({
        id: node.id,
