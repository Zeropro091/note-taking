...
export default function GraphView({
  ...
  const currentData: FGData = useMemo(() => {
    // Optimization: Pre-compute a Set of valid node IDs for O(1) lookup during edge filtering
    // This reduces edge filtering from O(N*E) to O(N+E) time complexity
    let filteredNodes = graph.nodes;
    let validNodeIds: Set<string> | null = null;

    if (filterTag) {
      filteredNodes = graph.nodes.filter((node) => node.tags.includes(filterTag));
      validNodeIds = new Set(filteredNodes.map(n => n.id));
    }
    ...
    return {
      ...
      links: graph.edges
        .filter((edge) => {
          if (!filterTag || !validNodeIds) return true;
          // Only include edge if source OR target has the filter tag
