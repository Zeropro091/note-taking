...
describe('validateNoteId', () => {
  ...
  it('should explicitly reject all path traversal attempts that use parent directories', () => {
    // It was insecure to silently strip .. and allow 'folder/../../note'.
  ...
  it('should handle Linux backslash bypass traversal attempts', () => {
    expect(validateNoteId('\\\\..\\\\..\\\\etc\\\\passwd')).toBe(false);
  });
});
