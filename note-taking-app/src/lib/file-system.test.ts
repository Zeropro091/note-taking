...
describe('validateNoteId', () => {
  ...
  it('should reject path traversal attempts that escape the directory', () => {
    // 'folder/../../note' escapes the base directory (relative is '../note.md')
    expect(validateNoteId('folder/../../note')).toBe(false);
  });

  it('should allow paths that resolve within the directory', () => {
    // 'folder/../note' resolves to 'note.md' within the base directory
    expect(validateNoteId('folder/../note')).toBe(true);
  });
