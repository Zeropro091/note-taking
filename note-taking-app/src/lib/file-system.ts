...
export function validateNoteId(id: string): boolean {
  ...
  // Normalize backslashes to forward slashes to prevent POSIX traversal bypasses
  const normalizedId = id.replace(/\\/g, '/');

  // Defense-in-depth: explicitly reject any traversal sequence using path separators
  // We use a regex to match '..' bounded by start/end of string or path separators
