...
export function validateNoteId(id: string): boolean {
  ...
  // to prevent POSIX traversal bypasses (e.g. `folder/..\\..\\etc\\passwd`)
  const normalizedId = id.replace(/\\/g, "/");

  // As a defense-in-depth measure, explicitly reject any IDs containing '..'
  // to prevent traversal attempts, even if they appear to resolve safely
