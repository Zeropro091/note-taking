...
export function validateNoteId(id: string): boolean {
  ...
  // Normalize backslashes to forward slashes to prevent Windows-style traversal bypasses
  const normalizedId = id.replace(/\\/g, '/');

  // Explicitly reject paths containing '..' to prevent directory traversal
  // This must be done before path resolution to catch strings that resolve within bounds but are still traversals
