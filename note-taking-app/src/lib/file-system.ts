...
export function validateNoteId(id: string): boolean {
  ...
  // Explicitly reject any path traversal attempts to be safe, defense in depth
  if (id.includes('..')) {
    return false;
  }

  // Normalize backslashes to forward slashes before resolving to prevent Windows POSIX bypass
