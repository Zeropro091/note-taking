...
export function validateNoteId(id: string): boolean {
  ...
  // Normalize backslashes to forward slashes before evaluation
  // to prevent POSIX traversal bypasses
  const normalizedId = id.replace(/\\/g, "/");

  // Explicitly reject any traversal attempts containing '..'
  // as a defense-in-depth measure, even if they might resolve safely
