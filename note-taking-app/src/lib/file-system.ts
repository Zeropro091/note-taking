...
export function validateNoteId(id: string): boolean {
  ...
  // Normalize backslashes to forward slashes before resolving to prevent POSIX traversal bypasses
  const normalizedId = id.replace(/\\/g, '/');

  // Explicitly reject '..' traversal attempts before path resolution.
  // We use bounded regex to avoid rejecting valid consecutive dots (e.g., "my..note").
