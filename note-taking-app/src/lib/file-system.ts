...
export function validateNoteId(id: string): boolean {
  ...
  // Explicitly reject any attempts at directory traversal via '..'
  if (id.includes("..")) {
    return false;
  }

  // Normalize backslashes to forward slashes before resolving to prevent
  // Windows-specific traversal bypasses when running on POSIX systems
