...
export function validateNoteId(id: string): boolean {
  ...
  // Defense-in-depth: Explicitly reject any path traversal attempts containing '..'
  // Even if they resolve safely, this is suspicious behavior.
  if (id.includes('..')) {
    return false;
  }

  // Normalize backslashes to forward slashes before resolution to prevent
  // POSIX traversal bypasses on certain environments.
